...
class tictactoe(commands.Cog):
    ...
    async def startXO(self, ctx, *, target: nextcord.Member = None):
        ...
        if not ctx.message.mentions:
            await ctx.send(
                "You have to mention someone else in order to play... or do u want to play by yourself?\nhttps://tenor.com/9yK4.gif"
            )
            ...
            await ctx.send(
                "Hehe... u seriously mentioned me? It seems u don't have friends!\nhttps://tenor.com/QC0C.gif"
            )
            ...
            def checkState(board, counter, end, state, choice, turn, player):
                ...
                t = 0
                for i in range(3):
                    if board[t] == turn and board[t + 1] == turn and board[
                            t + 2] == turn:
                ...
                t = 0
                for i in range(3):
                    if board[t] == turn and board[t + 3] == turn and board[
                            t + 6] == turn:
                        statement = currentp.mention + " has won\nGAME OVER\n"
                        end = 1
                        break
            ...
            while state == 1:
                if counter == 0:
                    ...
                    await ctx.send(currentp.mention +
                                   ", Do u want to be X or O?(0/1)")
                    while not valid:
                ...
                board, statement, end, counter, choice, state, turn = checkState(
                    board, counter, end, state, choice, turn, player)
                if statement:
